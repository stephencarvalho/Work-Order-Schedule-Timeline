...
const nextStartByCenter = new Map<string, Date>();
const seedYear = today.getFullYear();
const GENERATED_CENTER_INDEXES = Array.from({ length: 88 }, (_unused, index) => index + 1); // wc-002..wc-089

function centerId(index: number): string {
  return `wc-${String(index + 1).padStart(3, '0')}`;
}

function centerName(index: number): string {
  return `Work Center ${String(index + 1).padStart(3, '0')}`;
}

export const SAMPLE_WORK_CENTERS: WorkCenterDocument[] = Array.from(
  { length: WORK_CENTER_COUNT },
  (_unused, index) => ({
...
const GENERATED_WORK_ORDERS: WorkOrderDocument[] = Array.from(
  { length: WORK_ORDER_COUNT },
  (_unused, index) => {
...
const WORK_CENTER_ONE_ORDERS: WorkOrderDocument[] = [
  {
    docId: 'wo-wc001-01',
    docType: 'workOrder',
    data: {
...
const CROSS_YEAR_EDGE_CASE_ORDERS: WorkOrderDocument[] = [
  {
